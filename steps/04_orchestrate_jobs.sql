...
create or alter task vacation_spots_update
  ...
  AS MERGE INTO vacation_spots USING (
    ...
    join silver.attractions att on att.geo_name = city.geo_name -- STEP 5: INSERT CHANGES HERE
  ...
  WHEN MATCHED THEN
    UPDATE SET
      ...
      , vacation_spots.korean_restaurant_cnt = harmonized_vacation_spots.korean_restaurant_cnt -- STEP 5: INSERT CHANGES HERE
  WHEN NOT MATCHED THEN 
    INSERT VALUES (
      ...
      , harmonized_vacation_spots.korean_restaurant_cnt -- STEP 5: INSERT CHANGES HERE
    );
    ...
    begin
      let options varchar := (
        ...
        where true
          ...
          and (zoo_cnt > 0 or aquarium_cnt > 0) -- STEP 5: INSERT CHANGES HERE
        limit 10);
      ...
      if (:options = '[]') then
        CALL SYSTEM$SEND_EMAIL(
            'email_integration',
            'jason.drew@snowflake.com', -- INSERT YOUR EMAIL HERE
